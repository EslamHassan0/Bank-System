...
static void getClients(sting fileName){
    ...
    static void getAdmins(sting fileName){
        ...
        {
            if (!out.is_open()) {
                std::cerr << "Error: Cannot open " << fileName << " for clearing." << std::endl;
            }
            out.close();
            std::ofstream outLast(lastIdFile, std::ios::trunc);
            if (!outLast.is_open()) {
                std::cerr << "Error: Cannot open " << lastIdFile << " for clearing." << std::endl;
            }
            outLast.close();
        }
}