...
class FilesHelper{
    ...
    if(out.is_open()){
        out << c.getId() << "," << c.getName() << "," << c.getPassword << "," << c.balance << "\n";
        out.close();
        }
    }
...
static void saveAdmin(const Admin& a, string fileName){
    ...
    if (out.is_open())
    {
        out << a.id << "," << a.name << "," << a.password << "," << a.salary << "\n";
        out.close();
    }
}
static void getClients(string fileName){
    ...
    static void getAdmins(string fileName){
        ...
        static void clearFile(string fileName,string lastIdFile)
        {
            ...
            out.close();
            fstream outLast(lastIdFile, std::ios::trunc);
            if (!outLast.is_open()) {
                cout << "Error: Cannot open " << lastIdFile << " for clearing." <<endl;
            }
            outLast.close();
        }
}